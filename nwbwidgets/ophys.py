...
def show_plane_segmentation_3d(plane_seg: PlaneSegmentation):
    ...
    dims = (
        np.array(
            [
                max(max(plane_seg["voxel_mask"][i][dim]) for i in range(nrois))
                for dim in ["x", "y", "z"]
            ]
        ).astype("int")
        + 1
    ...
    for icolor, color in enumerate(color_wheel):
        ...
        for isel in sel:
            ...
            vol[
                tuple(dat["x"].astype("int")),
                tuple(dat["y"].astype("int")),
                tuple(dat["z"].astype("int")),
            ] = 1
...
def show_plane_segmentation_2d(
    ...
    fig.update_layout(
        ...
        xaxis=dict(
            mirror=True, range=[0, plane_seg["image_mask"].shape[1]], constrain="domain"
        ),
...
class PlaneSegmentation2DWidget(widgets.VBox):
    def __init__(self, plane_seg: PlaneSegmentation, color_wheel=color_wheel, **kwargs):
        ...
        if len(self.categorical_columns) == 1:
            ...
            self.children = [
                show_plane_segmentation_2d(plane_seg, color_by=self.color_by, **kwargs)
            ]
            ...
            self.fig = show_plane_segmentation_2d(
                plane_seg, color_by=self.cat_controller.value, **kwargs
            )
            ...
            self.children = [
                show_plane_segmentation_2d(self.plane_seg, color_by=None, **kwargs)
            ]
