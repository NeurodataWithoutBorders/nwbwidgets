...
def plot_spectrum_figure(spectrum, channel_nos, frequency_nos):
    ...
    if "power" in spectrum.fields and "phase" in spectrum.fields:
        ...
        axs[0].semilogy(all_freqs[start_id:end_id], power_data[start_id:end_id, range_])
        axs[0].set_ylabel("Power")
        axs[1].plot(all_freqs[start_id:end_id], phase_data[start_id:end_id, range_])
        axs[1].set_ylabel("phase")
    elif "power" in spectrum.fields:
        fig, ax = plt.subplots()
        ax.set_xlabel("frequency")
        ax.semilogy(all_freqs[start_id:end_id], power_data[start_id:end_id, range_])
        ax.set_ylabel("Power")
    elif "phase" in spectrum.fields:
        fig, ax = plt.subplots()
        ax.plot(all_freqs[start_id:end_id], phase_data[start_id:end_id, range_])
        ax.set_ylabel("phase")
        ax.set_xlabel("frequency")
