...
def extract_timing_information_from_nwbfile(nwbfile: NWBFile, verbose: bool = False) -> dict:
    ...
    for object_id, object in nwbfile.objects.items():
        ...
        if isinstance(object, DynamicTable) and hasattr(object, "start_time") and hasattr(object, "stop_time"):
            ...
            intervals  = [{"start_time": start_time, "stop_time": stop_time} for start_time, stop_time in zip(start_time, stop_time)]
            timing_dict = dict(start_time=intervals[0]["start_time"], stop_time=intervals[-1]["stop_time"])
            object_info = dict(object_path=object_path, object_name=object_name, object_id=object_id, top_module=top_module)
            temporal_information_dict[object_name] = dict(timing_dict=timing_dict, info=object_info, intervals=intervals)
...
def generate_epoch_annotations(temporal_data_dict) -> list:
    ...
    for i, epoch in enumerate(intervals, start=1):
        ...
        annotation_y  = len(temporal_data_dict)  + len(temporal_data_dict) * 0.15
...
def generate_total_duration_traces(temporal_data_dict, styling_dict):
    ...
    for i, object_name in enumerate(styling_dict, start=1):
        ...
        trace = go.Scatter(x=[start_time, stop_time], 
                           y=[i, i], 
                           mode='lines',
...
def generate_time_grid_widget(temporal_data_dict: dict) -> go.FigureWidget:
    ...
    styling_dict_rest = {key: dict(color=color_map[temporal_data_dict[key]["info"]["top_module"]]) for key in sorted_keys}
    ...
    epoch_guide_button = dict(
                ...
                x=1.15,  # Position from left (0 to 1)
                y=1.15,  # Position from bottom (0 to 1)
                xanchor="right",  # Anchor point is on the right
                yanchor="top",  # Anchor point is at the top
            )
    ...
    fig.update_layout(
        xaxis_title="Time (s)",
        yaxis=dict(tickvals=tickvals, ticktext=ticktext, range=[0.5, len(ticktext) * 1.25], ticks="", showline=False, showticklabels=True, showgrid=True, gridwidth=0.1),
